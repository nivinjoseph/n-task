    ...
    dispose() {
        return __awaiter(this, void 0, void 0, function* () {
            if (!this._isDisposed) {
                ...
                yield Promise.all(this._taskWorkers.map(t => t.taskWorker.terminate()));
            }
        });
    }
    initialize() {
        n_util_1.Make.loop(() => {
            const taskWorker = new worker_threads_1.Worker(this._taskWorkerFile);
            const tpTaskWorker = {
                taskWorker,
                isBusy: false
            };
            taskWorker.on("message", (data) => {
                const id = data.id;
